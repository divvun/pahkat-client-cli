...
use pahkat_client::{StoreConfig, PackageStore, WindowsPackageStore};
use pahkat_client::types::{InstallTarget, Package};
...
fn init() -> WindowsPackageStore {
    let config = StoreConfig::load_or_default(true).expect("could not create default configuration");
...
pub type WindowsTarget = pahkat_client::types::InstallTarget;
